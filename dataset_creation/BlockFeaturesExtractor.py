...
class BlockFeaturesExtractor:
    ...
    def countTransfer(self):
        ...
        mips_mnemonics = [s.lower() for s in mips_mnemonics] #变小写
        arm_mnemonics = [s.lower() for s in arm_mnemonics]
        x86_mnemonics = [s.lower() for s in x86_mnemonics]
        ...
        for i in list(self.instructions):
            if self.architecture == 'x86':
                if str(i['mnemonic']).lower() in x86_mnemonics:#mnemonic助记符,取出指令中的助记符
                    count = count + 1
            elif self.architecture == 'mips':
                if str(i['mnemonic']).lower() in mips_mnemonics:
                    count = count + 1
    ...
    def extractConstansStrings(self): #字符串内容
        constants = 0
        strings = 0
        for i, ins in enumerate(self.instructions):
            if 'opex' not in ins:
                continue
            for operand in ins['opex']['operands']:#operands:操作数,取出指令中操作书的信息{'size': 8, 'rw': 1, 'type': 'reg', 'value': 'rbp'}
                if operand['type'] == 'imm': # 对操作数中的立即数进行处理
                    if 'disasm' in self.r2_disasm[i] and 'str.' in self.r2_disasm[i]['disasm']: #disasm是汇编指令,
                        # 汇编指令中包含字符串(这种指令非常少,正常的汇编指令应该不会包含这种,但是程序会断到这里并且找不到字符串str. 不知道原因)
                        strings += 1
                    elif operand['value'] in self.string_addr:#操作数中包含字符串的地址
                        strings += 1
                    else:
                        constants += 1
