...
class DatabaseFactory:
    ...
    @staticmethod
    def analyze_file(item): #这里进行分析二进制文件
        global pool_sem
        os.setpgrp()
        ...
        analyzer = RadareFunctionAnalyzer(filename, use_symbol) #初始化
        p = ThreadPool(1)
        res = p.apply_async(analyzer.analyze)#这里用多线程对二进制文件进行分析
        ...
        for func in result:
            DatabaseFactory.insert_in_db(db, pool_sem, result[func], filename, func)#把数据存入数据库
            #func为函数名，result[func]为每个函数里面的三个图的信息
